...
Contract for the `phase-15-runbook` branch. Source: post-plan extension — **not**
...
operational-narrative deliverable. Turns operational knowledge that already lives in
the codebase into a real runbook artifact for the next on-call engineer.
...
stories and give the next on-call engineer a real reference. The batch-drain
...
- **Runbook, not a changelog note.** Written for the next on-call engineer (detection +
  guard + generalization) — a real operational artifact, not a summary.
