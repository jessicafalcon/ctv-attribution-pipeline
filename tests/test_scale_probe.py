...
def test_committed_scaling_block_structural_columns_reproduce() -> None:
    ...
    reproduce from the real tiers today — the guard the Phase-16 coherence audit
    found missing (SMALL_TIERS pins the properties, not the committed numbers).
    The `engine_join_state_current` column is deliberately NOT compared: it is
    last-household-wins, i.e. one arbitrary household's occupancy that moves with
    iteration order (BACKLOG, Phase-18 trigger) — pinning it would pin an arbitrary
    number."""
    from streaming.scale_probe import _BEGIN, _END, SCALING_MD
    ...
    points = run_curve(TIERS)
    assert len(rows) == len(points) == len(TIERS)
    for row, p in zip(rows, points, strict=True):
        assert row[:3] == [
            f"{p.exposures_in_window:,}",
            f"{p.structural_bytes / 1e6:.1f} MB",
            f"{p.bytes_per_exposure:.0f} B",
        ]
