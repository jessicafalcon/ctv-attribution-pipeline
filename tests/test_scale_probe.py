...
Small tiers (SMALL_TIERS) prove the PROPERTIES cheaply; one test at the end runs
the real 1k/10k/100k curve (~7 s) to prove the committed `docs/SCALING.md` block's
structural columns still reproduce — the guard the Phase-16 coherence audit found
missing. Exact-match by design: a CPython / pydantic bump that moves
`sys.getsizeof` is a "re-run `make scale-curve` and commit" chore, and this test is
what tells you to.
"""
...
def _structural(p: CurvePoint) -> tuple[int, int, int, float, int, int]:
    ...
    here FOR RE-RUN DETERMINISM — that is the rule the tracemalloc blocker taught:
    an unpinned rendered column is how nondeterminism slips into the committed doc.
    So this covers all four table columns (exposures_in_window, structural_bytes,
    bytes_per_exposure, join_state_current) plus join_state_peak. Only
    tracemalloc_peak_bytes is excluded, and only because it is console-only, never
    written to the doc. Distinct, weaker guard below: the committed block is
    compared AGAINST THE DOC only on the three structural columns —
    join_state_current is re-run-deterministic but last-household-wins (an
    arbitrary household's occupancy; BACKLOG, Phase-18 trigger), so it is never
    pinned against a committed number."""
    return (
        p.n_exposures,
...
def test_committed_scaling_block_structural_columns_reproduce() -> None:
    """The committed `docs/SCALING.md` block is regenerated by `make scale-curve`;
    its STRUCTURAL columns (exposures in window, structural MB, bytes/exposure) must
    reproduce from the real tiers today. Rendered through the generator's own
    `render_table`, so a format change cannot desync this test from the doc. The
    `engine_join_state_current` column is deliberately NOT compared: it is
    last-household-wins — one arbitrary household's occupancy that moves with
    iteration order (BACKLOG, Phase-18 trigger) — so pinning it would pin an
    arbitrary number. Failing here means: re-run `make scale-curve`, commit."""

    def _structural_cells(table: str) -> list[list[str]]:
        return [
    ...
    committed = SCALING_MD.read_text().split(_BEGIN, 1)[1].split(_END, 1)[0]
    live = render_table(run_curve(TIERS))
    assert len(_structural_cells(committed)) == len(TIERS)
    assert _structural_cells(committed) == _structural_cells(live)
