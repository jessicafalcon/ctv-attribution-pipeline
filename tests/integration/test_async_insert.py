...
def _attr_rows(n: int) -> list[AttributedConversion]:
    base = datetime(2026, 8, 1, tzinfo=UTC)
    return [
        AttributedConversion(
            conversion_id=f"c-{i:06d}",
            event_time=base + timedelta(minutes=i),
            ingest_time=base + timedelta(minutes=i, seconds=5),
            device_id=f"d-{i % 7}",
            ip=f"10.0.0.{i % 254}",
            conversion_type="purchase",
            revenue=round(1.5 * (i % 9), 2),
            order_id=f"o-{i}",
            household_id=f"h-{i % 7:04d}",
            resolution="device",
            ambiguous=False,
            ...
            attributed=True,
            path="hot",
            processed_at=base + timedelta(minutes=i, seconds=10),
            reason=None,
...
def _insert_attr(
    client, table: str, rows: list[AttributedConversion], async_insert: bool
) -> None:
    settings = _async_settings(async_insert) or None
    for chunk in batched(rows, 256):
        client.insert(
...
@pytest.fixture
def attr_probe_tables():
    client = connect()
    sync_t, async_t = "async_attr_probe_sync", "async_attr_probe_async"
    for t in (sync_t, async_t):
        client.command(f"drop table if exists {t}")
        client.command(f"create table {t} as attributed_conversions")
    yield client, sync_t, async_t
    for t in (sync_t, async_t):
        client.command(f"drop table if exists {t}")


def test_attributed_rows_are_byte_identical_with_async_on_and_off(
    attr_probe_tables,
) -> None:
    # Invariant 1 is over BOTH serving tables (review-round-1 #2): insert_attributed
    # carries the same async flag, so the attributed table must be proven directly too.
    client, sync_t, async_t = attr_probe_tables
    rows = _attr_rows(N)
    _insert_attr(client, sync_t, rows, async_insert=False)
    _insert_attr(client, async_t, rows, async_insert=True)
    for t in (sync_t, async_t):
        client.command(f"optimize table {t} final")

    sync_rows = client.query(
        f"select * from {sync_t} order by conversion_id"
    ).result_rows
    async_rows = client.query(
        f"select * from {async_t} order by conversion_id"
    ).result_rows
    assert async_rows == sync_rows
    assert len(async_rows) == N
