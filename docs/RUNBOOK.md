...
BASE DELETED` in the Makefile (make has no reason to expand what it will not
export), alongside `$(call _Q,$(value VAR))` on every recipe (the recipe-time
vector `-n` does show). If you audit a recipe's user value, quote it through
