...
**Measured** (`make scale-curve`, offline engine drain over the `scale_curve` profile): the retained hot-window state costs **~571 B/exposure** (571–573 B across the curve, structural — deep `sys.getsizeof` of the retained exposures ÷ entry count; deterministic on re-run). This replaces the old ~200 B guess.

| exposures in window | structural state | bytes/exposure (measured) | `engine_join_state_current` | `tracemalloc` peak (cross-check) |
|---|---|---|---|---|
| 1,000 | 0.6 MB | 573 B | 38 | 0.6 MB |
| 10,000 | 5.7 MB | 572 B | 50 | 4.1 MB |
| 100,000 | 57.1 MB | 571 B | 40 | 43.2 MB |

Occupancy (entries in window) scales linearly with the event count, and the per-exposure cost is flat across the curve — exactly the `exposure_rate × window × per-exposure-bytes` shape. `tracemalloc` peak is shown only as an independent cross-check; it is allocation-nondeterministic and is not the asserted number.
