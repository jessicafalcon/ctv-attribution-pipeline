...
- `fix/make-quote-profile` (PR #36, in review) — every recipe quotes user variables through `$(call _Q,$(value VAR))` and `unexport`s the six of them, closing both the recipe-time and the make-startup `$(shell …)` expansion vectors (`make -n` shows only the first). Phase-17 residual, closed.
