...
def rendered_summary(alert: str, *, table: str, value: int) -> str:
    """The alert's OWN `summary` annotation, read out of alerts.yml and rendered with
    ...
    text = " ".join(lines)
    return text.replace("{{ $labels.table }}", table).replace(
        "{{ $value }}", str(value)
    )
...
def render_synthetic() -> str:
    ...
    repo runs leaves a table anywhere near it — the real captures peak at 5 parts, so
    ...
    summary = rendered_summary(
        "PartCountHigh", table="attributed_conversions", value=PART_THRESHOLD + 1
    )
    return "\n".join(
        [
            ...
            "# leaves a table near it (real captures peak at 5 active parts, see",
            ...
            "            exp_annotations:",
            # derived from alerts.yml, never copied — promtool compares exactly
